...
type OrderBy = 'title' | 'score' | 'popularity';
...
function getSelected(items: SortSelectItem[], orderBy: string, sort: string) {
  return (
    items.find((it) => it.value.orderBy === orderBy && it.value.sort === sort) ?? items[0]
  );
}
...
export default function SortSelect() {
  ...
  const comboWidth = useComboWidth({ inputElRef, items: filteredItems.map((x) => ({ label: x.label, value: x.label })) });
